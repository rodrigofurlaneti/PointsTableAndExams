...
const schema = z.object({
  ...
  mealTime:   z.string().min(1, 'Select meal time'),
});
...
export default function FoodLogPage() {
  ...
  const { mutate: removeItem } = useRemoveLogItem();
  ...
  const onSubmit = (data: FormData) => {
    addItem(data, { onSuccess: () => reset({ quantity: 1, mealTime: '12:00' }) });
  };
      ...
      <div className={styles.body}>
        ...
        <div className={styles.panel}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
            <div>
              <label style={{ fontSize: 'var(--text-caption-strong)', fontWeight: 600, color: 'var(--color-ink)', letterSpacing: '-0.224px', display: 'block', marginBottom: 4 }}>
                Food item
              </label>
              <select
                ...
                {...register('foodItemId')}
                aria-invalid={!!errors.foodItemId}
              >
                <option value="">Select…</option>
                {foodItems.map((fi) => (
                  <option key={fi.id} value={fi.id}>
                    {fi.name} — {fi.points}pts ({fi.servingSize})
                  </option>
                ))}
              </select>
        ...
        <div className={styles.list}>
          <div className={styles.listHeader}>
            Today's items ({log?.items.length ?? 0})
          </div>
            ...
            log?.items.map((item) => (
              <div key={item.id} className={styles.listItem}>
                <div style={{ flex: 1 }}>
                  ...
                  <p className={styles.itemMeta}>{item.category} · ×{item.quantity} · {item.mealTime}</p>
                </div>
                <span className={styles.itemPoints}>{item.pointsComputed}pt</span>
                <button
                  className={styles.removeBtn}
                  onClick={() => removeItem(item.id)}
                  aria-label={`Remove ${item.foodItemName}`}
                  title="Remove"
                >
                  ×
                </button>
